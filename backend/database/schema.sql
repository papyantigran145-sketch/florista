...
-- Сообщения чата техподдержки.
-- room_id — идентификатор диалога (sessionId из socket.io handshake).
-- sender_id — users.id, если писал авторизованный пользователь; NULL для гостей.
-- deleted — мягкое удаление: строка остаётся, но текст не отдаётся клиенту.
