...
// ─── Безопасность: ключи ──────────────────────────────────────────────────
// ADMIN_KEY НЕ имеет значения по умолчанию. Если переменная не задана,
// вход по заголовку x-admin-key полностью отключён — иначе ключ-заглушка
// из исходников давал бы права администратора кому угодно.
const ADMIN_KEY    = process.env.ADMIN_KEY || null;
const TOKEN_SECRET = process.env.TOKEN_SECRET || null;
const TOKEN_TTL_MS = 1000 * 60 * 60 * 12; // 12 часов

if (!ADMIN_KEY) {
  console.warn('⚠️  ADMIN_KEY не задан в окружении — доступ по заголовку x-admin-key ОТКЛЮЧЁН.');
  console.warn('    Задайте ADMIN_KEY в .env / переменных окружения хостинга, чтобы включить вход по ключу.');
} else if (ADMIN_KEY.length < 16) {
  console.warn(`⚠️  ADMIN_KEY слишком короткий (${ADMIN_KEY.length} символов) — используйте минимум 16.`);
}

if (!TOKEN_SECRET) {
  console.error('⛔  TOKEN_SECRET не задан — подписывать токены нечем, вход в аккаунты работать не будет.');
  console.error('    Сгенерируйте секрет: node -e "console.log(require(\'crypto\').randomBytes(48).toString(\'hex\'))"');
}

// Сравнение секретов за постоянное время — обычный === даёт возможность
// подобрать ключ посимвольно по времени ответа.
...
// Проверка ключа администратора. Пустой ADMIN_KEY → всегда false.
function isAdminKey(candidate) {
  if (!ADMIN_KEY || !candidate) return false;
  return safeEqual(String(candidate), ADMIN_KEY);
}

// ─── Простые подписанные токены (HMAC, без внешних зависимостей) ─────────
function signToken(payload) {
  if (!TOKEN_SECRET) throw new Error('TOKEN_SECRET не настроен на сервере');
...
function verifyToken(token) {
  try {
    ...
    // safeEqual сам обрабатывает разную длину — timingSafeEqual на ней бросает RangeError
...
function attachUser(req, _res, next) {
  ...
  // id: null — это не аккаунт в таблице users, поэтому личные роуты
  // (аватар, карты) для него закрыты, а админские — открыты.
  if (!req.user && isAdminKey(req.headers['x-admin-key'])) {
    req.user = { id: null, role: 'admin', name: 'Key Admin', viaKey: true };
  }
  next();
}
...
// Требует НАСТОЯЩИЙ аккаунт: у входа по ключу id = null, и запись
// в user_cards/users с таким id всё равно упала бы на внешнем ключе.
...
// ─── Чат техподдержки: хранение в MySQL/TiDB ──────────────────────────────
// Раньше сообщения жили в `const chatRooms = new Map()` и полностью терялись
// при каждом рестарте или деплое. Теперь всё идёт через chat-store.js.

// Определяем, кто подключился по сокету: обычный токен или ключ администратора.
// Токен передаётся в handshake — auth.token (предпочтительно) или query.token.
function socketIdentity(socket) {
  ...
  const key = auth.adminKey || query.adminKey;
  if (isAdminKey(key)) return { id: null, role: 'admin', name: 'Key Admin', viaKey: true };
...
async function broadcastSessions(socket) {
  try {
    socket.emit('admin:sessions', await listSessions());
  } catch (err) {
    console.error('[chat] listSessions:', err.message);
    socket.emit('chat:error', { message: 'Не удалось загрузить список диалогов' });
  }
}
...
io.on('connection', async (socket) => {
  ...
  // id аккаунта пишем в sender_id; у гостей и входа по ключу его нет
  const senderId = Number.isInteger(user?.id) && user.id > 0 ? user.id : null;

  socket.join(sessionId);

  try {
    socket.emit('chat:history', await getHistory(sessionId));
  } catch (err) {
    console.error('[chat] history:', err.message);
    socket.emit('chat:error', { message: 'История чата временно недоступна' });
  }

  socket.on('admin:join', async () => {
    // Без этой проверки любой посетитель сайта мог подписаться на admin-room
    // и читать переписку всех остальных клиентов.
    if (!staff) return socket.emit('chat:error', { message: 'Нет доступа к панели оператора' });
    socket.join('admin-room');
    await broadcastSessions(socket);
  });

  socket.on('admin:watchSession', async ({ targetSessionId }) => {
    if (!staff || !targetSessionId) return;
    socket.join(targetSessionId);
    try {
      socket.emit('chat:history', await getHistory(targetSessionId));
    } catch (err) {
      console.error('[chat] watchSession:', err.message);
    }
  });

  socket.on('admin:markRead', async ({ targetSessionId }) => {
    if (!staff || !targetSessionId) return;
    try {
      await markRoomRead(targetSessionId);
  ...
  socket.on('admin:send', async ({ targetSessionId, text }) => {
    if (!staff) return socket.emit('chat:error', { message: 'Нет доступа к панели оператора' });
    ...
    try {
      ...
      io.to(targetSessionId).emit('receive_message', msg); // алиас для snake_case-клиентов
      io.to('admin-room').emit('admin:newMessage', { sessionId: targetSessionId, msg });
    } catch (err) {
      console.error('[chat] admin:send:', err.message);
      socket.emit('chat:error', { message: 'Сообщение не сохранилось, попробуйте ещё раз' });
    }
  });

  // Отправка сообщения клиентом. Один обработчик на два имени события:
  // 'chat:send' — то, что шлёт текущий фронтенд (LiveChat.js),
  // 'send_message' — алиас в snake_case. Переименовывать нельзя:
  // LiveChat.js слушает именно 'chat:message'.
  async function handleClientSend({ text, from } = {}) {
    if (!text?.trim()) return;
    // Роль берём с сервера: клиент не может объявить себя оператором,
    // просто прислав from: 'admin'.
    const senderRole = (from === 'admin' && staff) ? 'admin' : 'client';
    try {
      const msg = await saveMessage({ roomId: sessionId, senderId, senderRole, text: text.trim() });
      io.to(sessionId).emit('chat:message', msg);
      io.to(sessionId).emit('receive_message', msg); // алиас для snake_case-клиентов
      io.to('admin-room').emit('admin:newMessage', {
        sessionId,
        msg,
        unread: await countUnread(sessionId),
      });
    } catch (err) {
  ...
  socket.on('chat:send',    handleClientSend);
  socket.on('send_message', handleClientSend);

  socket.on('chat:edit', async ({ msgId, newText, targetSessionId }) => {
    if (!newText?.trim()) return;
    // Чужую комнату указывать разрешено только оператору; клиент правит
    // только свои сообщения и только в своей комнате.
    const roomId = staff ? (targetSessionId || sessionId) : sessionId;
    try {
      const ok = await editMessage({
        roomId,
        msgId,
        text:        newText.trim(),
        allowedRole: staff ? null : 'client',
      });
      if (!ok) return;
      io.to(roomId).emit('chat:edited', { msgId, newText: newText.trim(), roomId });
      io.to('admin-room').emit('chat:edited', { msgId, newText: newText.trim(), roomId });
    } catch (err) {
      console.error('[chat] chat:edit:', err.message);
    }
  });

  socket.on('chat:delete', async ({ msgId, targetSessionId }) => {
    const roomId = staff ? (targetSessionId || sessionId) : sessionId;
    try {
      const ok = await deleteMessage({ roomId, msgId, allowedRole: staff ? null : 'client' });
      if (!ok) return;
      io.to(roomId).emit('chat:deleted', { msgId, roomId });
      io.to('admin-room').emit('chat:deleted', { msgId, roomId });
    } catch (err) {
      console.error('[chat] chat:delete:', err.message);
    }
  });

  socket.on('disconnect', (reason) => {
    // socket.io сам выводит сокет из всех комнат при разрыве — руками
    // ничего чистить не надо. Логируем причину: 'transport close' —
    // обычный уход со страницы, 'ping timeout' — проблемы со связью.
    console.log(`[chat] отключение room=${sessionId} staff=${staff} причина=${reason}`);
  });
});
...
app.post('/api/auth/admin-key', (req, res) => {
  const { key } = req.body || {};
  if (!ADMIN_KEY)
    return res.status(503).json({ success: false, message: 'Вход по ключу отключён: ADMIN_KEY не настроен на сервере' });
  if (!isAdminKey(key))
    return res.status(401).json({ success: false, message: 'Неверный ключ доступа' });
  const token = signToken({ id: null, role: 'admin', name: 'Администратор (ключ)' });
  res.json({ success: true, data: { id: null, name: 'Администратор (ключ)', role: 'admin' }, token });
});
...
// ─── GEMINI AI — генерация товаров ───────────────────────────────────────
// POST /api/ai/generate-products
// Body: { prompt: "5 букетов с тюльпанами" }
// Ответ: { success: true, data: [ { name, price, old_price, discount, description, image_url } ] }
app.post('/api/ai/generate-products', async (req, res) => {
  try {
    if (!process.env.GEMINI_API_KEY) {
      return res.status(503).json({
        success: false,
        message: 'GEMINI_API_KEY не задан в .env. Добавьте ключ и перезапустите сервер.',
      });
    }

    const prompt = String(req.body?.prompt || '').trim();
    if (!prompt) {
      return res.status(400).json({ success: false, message: 'Поле prompt обязательно' });
    }

    const products = await generateProducts(prompt);

    // Дополнительная защита: убеждаемся, что это массив
    if (!Array.isArray(products)) {
      return res.status(500).json({ success: false, message: 'Gemini вернул некорректный ответ' });
    }

    return res.json({ success: true, data: products });
  } catch (err) {
    console.error('[Gemini] Ошибка:', err.message);
    return res.status(500).json({ success: false, message: err.message || 'Ошибка при обращении к Gemini API' });
  }
});
...
// ─── 404 — ловим всё, что не подошло ни под один роут ─────────────────────
// Путь НЕ указываем: в Express 5 (path-to-regexp v8) синтаксис '*' и '(.*)'
// вызывает PathError. Middleware без пути ловит все методы и все URL.
app.use((req, res) => {
  res.status(404).json({ success: false, message: `Not found: ${req.method} ${req.originalUrl}` });
});

// ─── Error handler ────────────────────────────────────────────────────────
// Express 5 сам ловит ошибки из async-роутов и присылает их сюда.
app.use((err, req, res, _next) => {
  const status = err.status || 500;
  // Логируем всегда: без этого 500-е уходят клиенту молча, и в pm2 logs пусто.
  if (status >= 500) console.error(`[500] ${req.method} ${req.originalUrl}:`, err.stack || err.message);
  res.status(status).json({
    success: false,
    // Наружу отдаём текст ошибки только для 4xx; детали 500-й остаются в логе
    message: status >= 500 ? 'Internal server error' : (err.message || 'Ошибка запроса'),
  });
});

// ─── Глобальная обработка ошибок процесса ─────────────────────────────────
let shuttingDown = false;

async function shutdown(reason, code = 0) {
  if (shuttingDown) return;
  shuttingDown = true;
  console.log(`⏹  Остановка сервера (${reason})...`);

  // Страховка: если что-то зависло, всё равно выходим через 10 секунд
  const force = setTimeout(() => {
    console.error('⛔  Не уложились в 10 секунд — принудительный выход');
    process.exit(code || 1);
  }, 10_000);
  force.unref();

  try {
    // io.close() разрывает сокеты и закрывает привязанный HTTP-сервер,
    // дождавшись завершения текущих запросов
    await new Promise((resolve) => io.close(resolve));
    await pool.end();
    console.log('✅  Соединения закрыты, пул MySQL освобождён');
  } catch (err) {
...
// Забытый .catch() не должен ронять магазин — логируем и работаем дальше
process.on('unhandledRejection', (reason) => {
  console.error('⛔  Unhandled Rejection:', reason instanceof Error ? reason.stack : reason);
});

// А вот после uncaughtException состояние процесса недоверенное:
// корректно закрываемся, pm2 / Render поднимут заново
process.on('uncaughtException', (err) => {
  console.error('⛔  Uncaught Exception:', err.stack || err.message);
  shutdown('uncaughtException', 1);
});

// Graceful shutdown при деплое и Ctrl+C
process.on('SIGTERM', () => shutdown('SIGTERM', 0));
process.on('SIGINT',  () => shutdown('SIGINT', 0));

// Ошибки самого HTTP-сервера (например, занятый порт) — без этого
// процесс падает с невнятным стектрейсом
httpServer.on('error', (err) => {
  if (err.code === 'EADDRINUSE') console.error(`⛔  Порт ${PORT} уже занят другим процессом`);
  else console.error('⛔  Ошибка HTTP-сервера:', err.message);
  process.exit(1);
});

// Готовим таблицу чата до приёма соединений: если её нет, создаём.
await initChatStore();
