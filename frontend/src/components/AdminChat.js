...
function AdminBubble({ m, onEdit, onDelete }) {
  ...
  if (m.deleted) {
    return (
      <div style={{ display: 'flex', justifyContent: own ? 'flex-end' : 'flex-start', marginBottom: '2px' }}>
        <div style={{ fontSize: '.75rem', color: '#777', fontStyle: 'italic', padding: '4px 10px', border: '1px dashed #555', borderRadius: '10px' }}>
          Сообщение удалено
        </div>
      </div>
    );
  ...
  return (
    <div
      style={{ display: 'flex', justifyContent: own ? 'flex-end' : 'flex-start', position: 'relative', marginBottom: '2px' }}
        ...
        {editing ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', minWidth: '200px' }}>
            ...
            <div style={{ display: 'flex', gap: '5px', justifyContent: 'flex-end' }}>
              <button
                onClick={() => { onEdit(m.id, val); setEdit(false); }}
                style={{ padding: '3px 10px', background: '#c0474a', color: '#fff', border: 'none', borderRadius: '7px', fontSize: '.78rem', cursor: 'pointer' }}
              >
                Сохранить
              </button>
              <button
                onClick={() => { setEdit(false); setVal(m.text); }}
                style={{ padding: '3px 8px', background: 'none', border: '1px solid #555', borderRadius: '7px', fontSize: '.78rem', cursor: 'pointer', color: '#aaa' }}
              >
                Отмена
              </button>
            </div>
              ...
              >
                {own && (
                  <button
                    ...
                    title="Редактировать"
                    style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: '12px', padding: '1px 3px' }}
                  >
                    <FiEdit2 />
                  </button>
                )}
                <button
                  ...
                  title="Удалить"
                  style={{ background: 'none', border: 'none', color: '#f87171', cursor: 'pointer', fontSize: '12px', padding: '1px 3px' }}
                >
                  <FiTrash2 />
                </button>
              </div>
...
export default function AdminChat() {
  ...
  useEffect(() => {
    console.log('🔌 Подключение к сокету:', API);
    
    const socket = io(API, {
      query: { sessionId: 'admin-' + Date.now() },
    ...
    socket.on('connect', () => {
      console.log('✅ Админ подключен к сокету');
      setConnected(true);
      socket.emit('admin:join');
    });

    socket.on('disconnect', () => {
      console.log('❌ Админ отключен от сокета');
      setConnected(false);
    });
    ...
    socket.on('chat:history', (history) => {
      console.log('📜 Получена история:', history.length);
      setMsgs(history);
    });
  ...
  useEffect(() => {
    ...
    console.log('👀 Наблюдение за сессией:', active);
  ...
  const send = useCallback(() => {
    ...
    console.log('📤 Отправка сообщения в сессию:', active);
      ...
      <div style={{ width: '270px', borderRight: border, display: 'flex', flexDirection: 'column', flexShrink: 0, background: 'var(--surface-2,#252118)' }}>
        <div style={{ padding: '16px 18px', borderBottom: border }}>
          <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--ink,#f0ece8)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {totalUnread > 0 && (
              <span style={{ background: '#c0474a', color: '#fff', borderRadius: '99px', padding: '1px 8px', fontSize: '.72rem', fontWeight: 700 }}>
                {totalUnread}
              </span>
            )}
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {sessions.length === 0 ? (
            <div style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--ink-3,#888)', fontSize: '.85rem' }}>
              Нет активных чатов
            </div>
          ) : (
            sessions.map(s => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  {s.unread > 0 && (
                    <span style={{ background: '#c0474a', color: '#fff', borderRadius: '99px', padding: '1px 7px', fontSize: '.7rem', fontWeight: 700, flexShrink: 0 }}>
                      {s.unread}
                    </span>
                  )}
                </div>
        ...
        {!active ? (
          ...
          <>
            <div style={{ padding: '13px 20px', borderBottom: border, background: 'rgba(192,71,74,0.07)', display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
              <div style={{ width: '34px', height: '34px', borderRadius: '50%', background: 'linear-gradient(135deg,#c0474a,#8b2e30)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', color: '#fff' }}>
                <FiUser />
              </div>
              <div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {msgs.length === 0 && (
                <div style={{ textAlign: 'center', color: 'var(--ink-3,#888)', fontSize: '.85rem', marginTop: '32px' }}>
                  Нет сообщений
                </div>
              )}
              {msgs.map(m => (
            ...
            <div style={{ padding: '12px 16px', borderTop: border, display: 'flex', gap: '8px', flexShrink: 0 }}>
              <textarea
                ...
                style={{
                  flex: 1,
                  resize: 'none',
                  border: '1px solid var(--border,#3d3830)',
                  borderRadius: '10px',
                  padding: '8px 14px',
                  fontSize: '.88rem',
                  fontFamily: 'inherit',
                  outline: 'none',
                  background: 'var(--surface-2,#252118)',
              ...
              <button
                ...
                style={{
                  padding: '0 18px',
                  background: text.trim() && active ? 'linear-gradient(135deg,#c0474a,#8b2e30)' : 'rgba(255,255,255,0.06)',
                  color: text.trim() && active ? '#fff' : 'var(--ink-3,#888)',
                  border: 'none',
                  borderRadius: '10px',
                  cursor: text.trim() && active ? 'pointer' : 'default',
                  fontSize: '.9rem',
                  fontWeight: 600,
                  whiteSpace: 'nowrap'
                }}
